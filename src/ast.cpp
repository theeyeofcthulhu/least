...
const std::map<arit_op, std::string> arit_str_map = {
    std::make_pair(ADD, "+"), std::make_pair(DIV, "/"), std::make_pair(MOD, "%"),
    std::make_pair(MUL, "*"), std::make_pair(SUB, "-"),
};
                                 ...
                                 CompileInfo &c_info)
{
    ...
    for (size_t i = 0; i < args.size(); i++) {
        ...
        if (spec.types[i] == T_NUM_GENERAL) {
            ...
            c_info.err.on_false(arg->get_type() == T_VAR || arg->get_type() == T_CONST ||
                             ...
                             CompileInfo &c_info)
{
    ...
    for (size_t i = 0; i < ts.size(); i++) {
        ...
        if (expect_operator) {
            ...
            c_info.err.on_false(t->get_type() == lexer::TK_NUM || t->get_type() == lexer::TK_VAR ||
                                    t->get_type() == lexer::TK_CALL,
                                    ...
                                    CompileInfo &c_info)
{
    ...
    for (; ts[next_i]->get_type() != lexer::TK_EOL; next_i++) {
        if (ts[next_i]->get_type() == lexer::TK_LOG) {
            ...
            auto next =
                std::make_shared<Log>(ts[next_i]->get_line(), left, nullptr, log->get_log());
            if (res == nullptr) {
                res = next;
            } else {
                current->right = next;
