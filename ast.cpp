...
std::shared_ptr<ast::node> node_from_var_or_const(std::shared_ptr<lexer::token> tk,
                                                  compile_info &c_info) {
    c_info.err.on_false(tk->get_type() == lexer::TK_VAR || tk->get_type() == lexer::TK_NUM,
...
std::shared_ptr<node> parse_arit_expr(std::vector<std::shared_ptr<lexer::token>> ts,
                                      compile_info &c_info) {
...
std::shared_ptr<cmp> parse_condition(std::vector<std::shared_ptr<lexer::token>> ts,
                                     size_t &i, compile_info &c_info) {
...
parse_condition_to_while(std::vector<std::shared_ptr<lexer::token>> ts, size_t &i,
                         std::shared_ptr<n_body> root, compile_info &c_info) {
    std::shared_ptr<cmp> condition = parse_condition(ts, i, c_info);
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
        ...
        case lexer::TK_KEY:
        {
            ...
            case K_INT:
            {
                ...
                size_t next_sep;
                while ((next_sep = next_of_type_on_line(tokens, i, lexer::TK_SEP)) <
                       tokens.size()) {
                    switch (tokens[i]->get_type()) {
                    case lexer::TK_LSTR:
                    {
